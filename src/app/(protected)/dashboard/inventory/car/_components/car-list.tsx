...
export const CarList = ({ token }: CarListProps) => {
  ...
  const { data, isError, isLoading } = useCarGetAll({ params: params.toString() });

  const { mutate, isPending } = useMutation({
    mutationKey: ['deleteCar'],
    mutationFn: (id: string) => UtilsCarDelete({ id, token }),
  });
        ...
        <TableBody className="h-full">
          {data.data.length > 0 ? (
            data.data.map((car: Car) => {
              ...
              return (
                <TableRow key={car.id} className="odd:bg-gray-100 even:bg-white">
                  <TableCell>
                  ...
                  <TableCell>
                    ...
                    <span
                      className={`${
                        car.status === 'AVAILABLE' ? 'bg-green-200 text-green-700' : car.status === 'UNAVAILABLE' ? 'bg-red-200 text-red-700' : 'bg-gray-200 text-gray-700'
                  ...
                  <TableCell>
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <h4 className="text-base text-primary font-semibold">{car.brand.name}</h4>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-col items-center justify-center">
                  ...
                  <TableCell>
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <h4>
                        <span className="text-primary font-semibold">Rp {car.price_per_day}</span>
                        /days
                      </h4>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <ButtonDelete id={car.id} onclick={() => handleDelete(car.id)} isLoading={isPending}>
                        <FaTrash />
                      </ButtonDelete>
                    </div>
                  </TableCell>
