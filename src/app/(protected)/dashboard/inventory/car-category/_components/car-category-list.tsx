...
export const CarCategoryList = () => {
  ...
  const { mutate } = useMutation({
    ...
    mutationFn: (id: string) => UtilsCarCategory.delete({ id, token: session?.user.token }),
  });
      ...
      <Table className="rounded-t-md overflow-hidden">
        ...
        <TableBody>
          {data.data?.map((category, index: number) => (
            <TableRow key={category.id} className="border-none odd:bg-gray-100 even:bg-white">
              <TableCell>{index + 1}</TableCell>
              <TableCell className="font-medium">{category.name}</TableCell>
              <TableCell>
                <p className="max-w-[300px] break-words whitespace-normal line-clamp-2">{category.description}</p>
              </TableCell>
              <TableCell>
                <div className="flex items-center justify-end gap-x-3">
                  ...
                  <Button size="sm" variant="destructive" onClick={() => handleDelete(category.id, category.name)} className="text-xs">
                    <FaTrash />
                  </Button>
                </div>
              </TableCell>
